...
`microenvRF` is an R package for **tumor microenvironment (TME)** classification using a Random Forest model. It provides:

- **Cell type classification**: Cancer / T_Cell / Fibroblast  
- **Disease status classification**: Tumor vs Healthy_Control  

> Note (coursework requirement): the exported prediction function returns **class labels** (not probabilities).
...
The following figures are taken from the project report and summarize model performance:
